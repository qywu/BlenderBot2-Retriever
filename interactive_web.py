...
from blenderbot2.blenderbot2 import BlenderBot2FidAgent
...
class MyHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == '/interact':
            ...
            model_response = self._interactive_running(SHARED.get('opt'), body.decode('utf-8'))

            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            # cannot parse metrics
            del model_response['metrics']
...
if __name__ == '__main__':
    InteractiveWeb.main("--task", "blended_skill_talk", "--model-file", "zoo:blenderbot2/blenderbot2_3B/model",
                        "--search-server", "https://www.google.com")
