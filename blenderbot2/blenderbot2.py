...
from .web_retriever import SearchQueryWebRetriever
...
class BlenderBot2ModelTypeMixin(RagModelInterface):
    ...
    def __init__(self, opt: Opt, null_idx: int):
        ...
        if (KnowledgeAccessMethod(opt['knowledge_access_method']) is KnowledgeAccessMethod.ALL):
            self.n_docs *= 2


class BlenderBot2RagSequence(BlenderBot2ModelTypeMixin, RagSequence):

    def augment_batch_for_generation(self, batch: Batch, model: BlenderBot2RagModel) -> Batch:
        """
    ...
    def get_generation_input(
        self, batch: Batch
    ) -> Tuple[torch.LongTensor, Optional[torch.LongTensor], Optional[torch.Tensor], Optional[torch.Tensor],
...
class BlenderBot2RagAgent(RagAgent):
    ...
    def add_cmdline_args(cls, parser: ParlaiParser, partial_opt: Optional[Opt] = None) -> ParlaiParser:
        ...
        q_gen_group.add_argument('--query-generator-beam-size', type=int, default=1, help='SQ Gen Beam Size')
        q_gen_group.add_argument(
    ...
    @staticmethod
    def update_state_dict(opt: Opt, state_dict: Dict[str, torch.Tensor], model: torch.nn.Module):
        """
    ...
    def observe(self, observation: Union[Dict, Message]) -> Message:
        ...
        if ('query_generator_vec' not in observation and self.opt['query_generator_key'] in observation):
            self._set_query_generator_vec(observation)
        if 'gold_doc_vec' not in observation and all(k in observation for k in [
                self.opt['gold_document_key'],
                self.opt['gold_sentence_key'],
                self.opt['gold_document_titles_key'],
        ]):
            self._set_gold_doc_vec(observation)
        if ('memory_decoder_vec' not in observation and self.opt['memory_decoder_key'] in observation):
    ...
    def _filter_text(self, text: str, filter_phrase: str, delimiter: str = '\n') -> str:
        ...
        """
        split_text = [t for tt in text.split(self.opt.get('delimiter', '\n')) for t in tt.split('\n')]
    ...
    def _set_memory_vec(self, observation: Message) -> Message:
        ...
        if method in [
            ...
            if isinstance(memories, str):
                memories = [t for tt in memories.split(self.opt.get('delimiter', '\n')) for t in tt.split('\n')]
    ...
    def _set_query_generator_vec(self, observation: Message) -> Message:
        ...
        if (method in [
            ...
            if self.add_person_tokens:
                query_generator_input = self._remove_person_tokens(query_generator_input)
            query_generator_vec = self.model.tokenize_query_generator_input(query_generator_input)
    ...
    def _set_memory_decoder_vec(self, observation: Message) -> Message:
        ...
        if (method in [
            ...
            conv_lines = [
                t for tt in memory_decoder_input.split(self.opt.get('delimiter', '\n')) for t in tt.split('\n')
            ]
            memory_decoder_vec = [self.model.tokenize_memory_decoder_input(i) for i in conv_lines]
    ...
    def _set_batch_query_generator_vec(self, valid_exs: List[Message], batch: Batch) -> Batch:
        """
    ...
    def _set_batch_memory_decoder_vec(self, valid_exs: List[Message], batch: Batch) -> Batch:
        """
    ...
    def _model_input(
        self, batch: Batch
               ...
               torch.LongTensor, torch.LongTensor, torch.LongTensor, torch.LongTensor, torch.LongTensor,
    ...
    def compute_loss(self, batch: Batch, return_output: bool = False) -> Union[torch.Tensor, Tuple[torch.Tensor, Any]]:
        """
        ...
        if (KnowledgeAccessMethod(self.opt['knowledge_access_method']) is KnowledgeAccessMethod.CLASSIFY and
                self.model.has_query_generator()):
